...
function Tilemap:optimize_map()
  ...
  for x = 2, #self.map do
    for y, block in pairs(self.map[x]) do
      if self.map[x - 1][y] ~= nil and block.height == self.map[x - 1][y].height and block.type == self.map[x - 1][y].type then
...
function Tilemap:draw_map()
  ...
  for _, block in ipairs(self.static_blocks) do
    ...
    if block.map_info.type.r == BLOCK_TYPE.END.r then
      -- love.graphics.polygon('line', block.body:getWorldPoints(block.visual_points))
      love.graphics.push()
      love.graphics.translate(block.body:getX(), block.body:getY())
      -- love.graphics.rotate(block.body:getAngle())
      love.graphics.polygon('line', block.visual_points)
      love.graphics.pop()
    else
